...
// Function to partition for Deterministic Quick Sort (pivot = first element)
...
// Function to partition for Randomized Quick Sort (pivot = random element)
int randomizedPartition(vector<int> &arr, int low, int high, int &comparisons, int &swaps) {
    int randomIndex = low + rand() % (high - low + 1);
    swap(arr[low], arr[randomIndex]);
    swaps++;
    return deterministicPartition(arr, low, high, comparisons, swaps);
}
